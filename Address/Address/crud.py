...
class AdressBook:
    ...
    def get_address_by_coordinates(self,db: Session,map_latitude:float,map_longitude:float):
    ...
    def get_address_by_distance(self,db: Session,distance: int | None= None,logger = None):
        ...
        for a in address:
            ...
            logger.info(f'the given adress with latitude: {a.map_latitude} and longitude: {a.map_longitude} is at distance of {geopy.distance.geodesic(cord1, cord2).km}km')
    ...
    def get_address(self,db: Session):
        return db.query(models.Address).all()

    #to create a new address with latitute and longitude 
    def create_address(self,db: Session, address: schemas.Addresscreate):
    ...
    def update_address(self,id:int,db: Session, address: schemas.Addressupdate):
    ...
    def delete_address(self,id:int,db: Session):
        db_address = db.query(models.Address).filter(models.Address.id==id).first()
        if not db_address:
            raise HTTPException(status_code=404, detail="address not found")
